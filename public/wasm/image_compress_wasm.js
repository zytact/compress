...
export function resize_by_dimensions(data, width, height, format, quality) {
    ...
    const ret = wasm.resize_by_dimensions(
        ptr0,
        len0,
        width,
        height,
 ...
 * @returns {Uint8Array}
 */
export function resize_by_filesize(
    data,
    target_bytes,
    floor_quality,
    ceil_quality,
    ...
    const ptr0 = passArray8ToWasm0(data, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.resize_by_filesize(
        ptr0,
        len0,
        target_bytes,
        isLikeNone(floor_quality) ? 0xffffff : floor_quality,
        isLikeNone(ceil_quality) ? 0xffffff : ceil_quality,
        isLikeNone(_tolerance_percent)
            ? 0x100000001
            : Math.fround(_tolerance_percent),
    );
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
